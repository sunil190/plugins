<?php 
	$id = htmlspecialchars($_GET['subscriber_id']); 
	

	if (isset ($_GET['action']) && $_GET ['action'] == 'delete') {
		
		$wpdb->query('DELETE FROM wp_subscriber where id="'.$id.'" ');	
	} 
...
// admin section backend
	add_menu_page( 'Subscriber', 'Email Subscriber','manage_options', __FILE__, 'subscribe_main_page',plugins_url( '/images/wp-icon.png', __FILE__ ) );	
	 
// create setting submenu
	function subscribe_main_page(){
	global $wpdb;
	$pagination_count = $wpdb->get_var("SELECT COUNT('id') FROM ".$wpdb->prefix. "subscriber");
	...
	if($pagination_count > 0) {
		...
		$pag->limit($per_page);
		$pag->target("admin.php?page=subscriber/subscriber_paginate.php");
		...
		global $wpdb;
		$sub_results=$wpdb->get_results('SELECT * from wp_subscriber where id ='.$id); ?>	
			<table class="widefat">
						<thead>
						   <tr>
								<th>Sr. No.</th>
						...
						<tbody>
							...
							<form method="post"> 
								<td><input type="email" name="sub_email1" value="<?php echo $sub_results[0]->sub_email;?>" /></td>
								<td><input type="submit" name="submit" value="save"/></td>								
							</form>
							<td><a href="admin.php?page=subscriber/subscriber_paginate.php&action=edit&subscriber_id=<?php echo $record->id;?>">delete</a></td>							
							</tr>
								<?php
							}
						
							if (isset($_POST['submit']) && $_POST['submit'] == 'save'){	
							$sub_email1=$_POST['sub_email1'];
							global $wpdb;
								$wpdb->query('UPDATE `wp_subscriber` SET `sub_email`="'.$sub_email1.'" WHERE id ="'.$id.'" '); 	
								echo '<script type="text/javascript">'; 
								echo 'alert("Email Upadated Successfully");'; 
								echo 'window.location.href ="admin.php?page=subscriber/subscriber_paginate.php";';
								echo '</script>';
							}	
		...
		$records = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "subscriber LIMIT %d, %d", $list_start, $per_page));
		if($records) {
        ...
        <table class="widefat">
            ...
            <tbody>
				...
				echo"<td>". $a++ ."</td>";
				echo "<td>". $record->sub_name ."</td>";
				echo "<td>". $record->sub_email ."</td>";
				?>	
				<td><a href="admin.php?page=subscriber/subscriber_paginate.php&action=edit&subscriber_id=<?php echo $record->id;?>">Edit</a></td>
				<td><a href="admin.php?page=subscriber/subscriber_paginate.php&action=delete&subscriber_id=<?php echo $record->id;?>">delete</a></td>				
                </tr>
                    <?php
                }
                ?>
        ...
        ?>
            <div class="tablenav">
                ...
                <div class="tablenav-pages">
                    <span class="displaying-num"><?php echo $per_page; ?> items</span>
                    <?php $pag->show(); ?>
                </div>
            </div>
        <?php
